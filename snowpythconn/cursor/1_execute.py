...
cur.execute("PUT file:///root/airflow/data/employee.csv @DEMO_DB.PUBLIC.%EMP_PYTHON")

cur.execute("COPY INTO DEMO_DB.PUBLIC.EMP_PYTHON FROM @DEMO_DB.PUBLIC.%EMP_PYTHON")


#using file stream parameter
f = open("/root/airflow/data/employee.csv",'rb')

cur.execute("rm @DEMO_DB.PUBLIC.%EMP_PYTHON")
cur.execute("PUT file:///root/dummy/data/employee.csv @DEMO_DB.PUBLIC.%EMP_PYTHON",
file_stream=f)
